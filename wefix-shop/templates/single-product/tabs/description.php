...
$heading = esc_html( apply_filters( 'woocommerce_product_description_heading', esc_html__( 'Description', 'wefix-shop' ) ) );

?>

<?php if ( $heading ) : ?>
  <h2><?php echo wefix_html_output($heading); ?></h2>
<?php endif; ?>
...
if( $product_template == 'custom-template' ) {
	...
	if(isset($settings['description']) && !empty($settings['description'])) {
		...
		if($settings['description'] == 'custom-description' && isset($settings['custom-description']) && !empty($settings['custom-description'])) {
			...
			echo wefix_html_output($post_description);
