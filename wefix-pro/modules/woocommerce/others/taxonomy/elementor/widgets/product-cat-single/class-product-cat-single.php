...
class WeFix_Shop_Widget_Product_Cat_Single extends Widget_Base {
	...
	public function get_title() {
		return esc_html__( 'Product Category - Single', 'wefix-pro' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'product_cat_single_section', array(
			...
			$this->add_control( 'cat_id', array(
				'label'   => esc_html__( 'Category Id', 'wefix-pro' ),
				'type'    => Controls_Manager::TEXT,
				'description' => esc_html__( 'Provide category id to show.', 'wefix-pro' ),
				'default' => ''
			) );
			...
			$this->add_control( 'image', array(
				...
				'label'     => esc_html__( 'Image', 'wefix-pro' ),
				'description' => esc_html__( 'Update if you like to use alternate image for this category.', 'wefix-pro' ),
			) );
	...
	protected function render() {
		...
		if( is_array($categories) && !empty($categories) ) {
			...
			foreach( $categories as $category ) {
				...
				if($wefix_shop_cat_color != '') {
					$color_html .= '<div class="wdt-shop-category-listing-color" style="background-color:'.$wefix_shop_cat_color.';"></div>';
				}
					...
					$output .= '<div class="wdt-shop-category-listing-item '.$settings['type'].' '.$settings['class'].'">';
						$output .= '<div class="wdt-shop-category-listing-inner">';
							$output .= '<div class="wdt-shop-category-listing-image">';
								$output .= '<a href="'.get_term_link($category->term_id).'"><img src="'.esc_url($cat_image).'" alt="'.esc_html__('Category Image', 'wefix-pro').'" title="'.esc_html__('Category Image', 'wefix-pro').'" /></a>';
							$output .= '</div>';
						...
						$output .= '<div class="wdt-shop-category-listing-inner">';
							...
							$output .= '<div class="wdt-shop-category-meta-data">';
								...
								if($settings['show_starting_price'] == 'true') {
									...
									if($woo_price_html != '') {
										$output .= '<div class="wdt-shop-category-starting-price-html">'.sprintf(esc_html__('Starts from %1$s', 'wefix-pro'), $woo_price_html).'</div>';
									}
