...
class WeFix_Shop_Elementor_Single_Buy_Now_Widgets {
	...
	function __construct() {
		...
		$this->wefix_shop_load_bn_modules();
	 ...
	 * Init
	 */
	function wefix_shop_load_bn_modules() {

		require wefix_shop_single_module_buy_now()->module_dir_path() . 'elementor/utils.php';
	...
	function wefix_shop_register_widget_styles( $suffix ) {
		...
		wp_register_style( 'wdt-shop-buy-now',
			wefix_shop_single_module_buy_now()->module_dir_url() . 'assets/css/style'.$suffix.'.css',
			array()
		);
	...
	function wefix_shop_register_widget_scripts( $suffix ) {
		...
		wp_register_script( 'wdt-shop-buy-now',
			wefix_shop_single_module_buy_now()->module_dir_url() . 'assets/js/scripts'.$suffix.'.js',
			array( 'jquery' ),
			false,
			true
		);
...
WeFix_Shop_Elementor_Single_Buy_Now_Widgets::instance();