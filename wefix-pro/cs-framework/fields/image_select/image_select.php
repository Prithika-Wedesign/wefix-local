  ...
  public function output() {
    ...
    if( isset( $this->field['options'] ) ) {
      ...
      foreach ( $options as $key => $value ) {
        echo '<label><input type="'. esc_attr($input_type) .'" name="'. esc_attr($this->element_name( $input_attr )) .'" value="'. esc_attr($key) .'"'. wefix_html_output($this->element_class()) . $this->element_attributes( $key ) . $this->checked( $this->element_value(), $key ) .'/><img src="'. esc_url($value) .'" alt="'. esc_attr($key) .'" title="'. esc_attr($key) .'" /></label>';
      }
    }
