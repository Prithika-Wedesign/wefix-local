    ...
    public function render_html($widget_object, $settings) {
        ...
        if ($form_query->have_posts()) {
            ...
            while ($form_query->have_posts()) {
				...
				$delivery_hours  = !empty($service_settings['service-duration']) ? $service_settings['service-duration'] : '';

				$duration_label = '';
				if ($duration === 'day') {
					$duration_label = esc_html__(' / day', 'wdt-elementor-addon');
				} elseif ($duration === 'month') {
					$duration_label = esc_html__(' / month', 'wdt-elementor-addon');
				} elseif ($duration === 'year') {
					$duration_label = esc_html__(' / year', 'wdt-elementor-addon');
				}

				$active_class = '';
				if ( is_singular('wdt_services') && get_queried_object_id() == $service_id ) {
					$active_class = ' dt-service-active';
				}

                $output .= '<div class="'.esc_attr($module_layout_class).'">';
					$output .= '<div class="wdt-service-item wdt-service-' . esc_attr($service_type) . ' ' . esc_attr($active_class) . '">';
                        ...
                        if ($service_type === 'type-1') {
                            ...
                            $output .= '</div>';
  							...
  							$output .= '<div class="wdt-service-media-group">';
								...
								$output .= '<div class="wdt-service-image">';
                                	$output .= $this->render_service_image($service_id);
									$output .= '<div class="wdt-service-duration"><span>'.esc_html__('Estimated Delivery - ','wdt-elementor-addon').wefix_pro_duration_to_string($delivery_hours).'</span></div>';
								$output .= '</div>';
                            $output .= '</div>';
                            ...
                            $output .= '<div class="wdt-service-detail-group">';
								...
								$output .= '<div class="wdt-service-content-group">';
									$output .= '<div class="wdt-service-title"><h5>';
										$output .= '<a href="' . esc_url(get_permalink()) . '">' . get_the_title() . '</a>';
									$output .= '</h5></div>';
									$output .= '<div class="wdt-service-icon-group">';
