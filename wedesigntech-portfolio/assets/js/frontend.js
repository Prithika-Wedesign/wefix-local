var wdtPortfolioFrontendUtils = {
	...
	wdtPortfolioListingCarousel : function($scope, $) {
		...
		const $space_between = $carouselresponsive['space_between_gaps'];
		var $deviceMode = elementorFrontend.getCurrentDeviceMode();
        const $space_between_side = $space_between[$deviceMode] ? $space_between[$deviceMode] : 0;
		const $space_between_is=parseInt($space_between_side);
		...
		if ($carouselbulletpagination == true) {
			swiperOptions.pagination = {
				el: ".wdt-swiper-pagination-fraction-bullets",
				type: 'custom',
				clickable: true,
				renderCustom: function (swiper, current, total) {
					...
					return `
						<div class="wdt-swiper-pagination-fraction">${paddedCurrent}/${paddedTotal}</div>
						<div class="wdt-swiper-pagination-bullets">${bullets}</div>
					`;
				}
			};
		}
	...
	wdtPortfolioLoadDataOutput : function(output_container) {
		...
		var enable_carousel = keyword = list_items = categories = tags = startdate = features_query = features_total_query = orderby = others = module_id = '';
	...
	wdtPortfolioAjaxPagination : function() {
		...
		jQuery( 'body' ).delegate( '.wdt-listing-pagination a:not(.disabled)', 'click', function(e) {
			...
			var list_items = categories = tags = keyword = startdate = features_query = features_total_query = orderby = others = '';
	...
	wdtPortfolioListingImageSwiperGallery: function () {
		...
		jQuery('.wdt-listings-image-gallery-container').each(function () {
			...
			if ($mediacarouselmousewheelcontrol != ''){
				mediaswiperOptions.mousewheel = $mediacarouselmousewheelcontrol;
			}
