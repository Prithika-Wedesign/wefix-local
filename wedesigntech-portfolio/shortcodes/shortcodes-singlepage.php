		...
		function wdt_sp_contact_details( $attrs, $content = null ) {
			...
			if ($attrs['listing_id'] != '') {
				...
				$output .= '<div class="wdt-listings-contactdetails-container '.esc_attr($attrs['class']).'">';
					$output .= '<h4 class="wdt-listings-contactdetails-title">'.esc_html__( 'Client Details', 'wdt-portfolio' ).'</h4>';
					$output .= '<div class="wdt-listings-contactdetails-list">';

					// Date (static for now — replace with meta if needed)
					$output .= '<div class="wdt-listings-contactdetails-row wdt-detail-date"><span class="wdt-detail-label">Date:</span><span class="wdt-detail-value">' . date("F d, Y") . '</span></div>';

					if ($attrs['include_client'] == 'true') {
						$wdt_client = get_post_meta($attrs['listing_id'], 'wdt_client', true);
						if ($wdt_client != '') {
							$output .= '<div class="wdt-listings-contactdetails-row wdt-detail-client"><span class="wdt-detail-label">Client:</span><span class="wdt-detail-value">' . esc_html($wdt_client) . '</span></div>';
						}
					}

					if ($attrs['include_category'] == 'true') {
						$terms = get_the_terms($attrs['listing_id'], 'wdt_listings_category');
						if ($terms && !is_wp_error($terms)) {
							...
							if (!empty($term_links)) {
								$output .= '<div class="wdt-listings-contactdetails-row wdt-detail-category">
									<span class="wdt-detail-label">Category:</span>
									<span class="wdt-detail-value">' . implode(', ', $term_links) . '</span>
								</div>';
							}
						}
					...
					if ($attrs['include_estimation'] == 'true') {
						$wdt_estimation = get_post_meta($attrs['listing_id'], 'wdt_estimation', true);
						if ($wdt_estimation != '') {
							$output .= '<div class="wdt-listings-contactdetails-row wdt-detail-estimation"><span class="wdt-detail-label">Estimation:</span><span class="wdt-detail-value">' . esc_html($wdt_estimation) . '</span></div>';
						}
					}

					if ($attrs['include_place'] == 'true') {
						$wdt_place = get_post_meta($attrs['listing_id'], 'wdt_place', true);
						if ($wdt_place != '') {
							$output .= '<div class="wdt-listings-contactdetails-row wdt-detail-place"><span class="wdt-detail-label">Place:</span><span class="wdt-detail-value">' . esc_html($wdt_place) . '</span></div>';
						}
					}

					if ($attrs['include_duration'] == 'true') {
						$wdt_duration = get_post_meta($attrs['listing_id'], 'wdt_duration', true);
						if ($wdt_duration != '') {
							$output .= '<div class="wdt-listings-contactdetails-row wdt-detail-duration"><span class="wdt-detail-label">Duration:</span><span class="wdt-detail-value">' . esc_html($wdt_duration) . '</span></div>';
						}
					}

					if ($attrs['include_social'] == 'true') {
				...
				$listing_singular_label = apply_filters('listing_label', 'singular');
				$output .= sprintf(
					esc_html__('Please provide %1$s id to display corresponding data!', 'wdt-portfolio'),
					strtolower($listing_singular_label)
				);
			}
		...
		function wdt_sp_social_links( $attrs, $content = null ) {
			...
			if($attrs['listing_id'] != '') {
				...
				$output .= '<div class="wdt-listings-sociallinks-container '.esc_attr( $attrs['class'] ).'">';
                    ...
                    $output .= '<span>'.esc_html__('Share:', 'wdt-portfolio').'</span>';
		...
		function wdt_sp_taxonomy( $attrs, $content = null ) {
			...
			if($attrs['listing_id'] != '') {
				...
				if (isset($attrs['splice']) && $attrs['splice'] != '') {
					$listing_taxonomies = array_slice($listing_taxonomies, 0, intval($attrs['splice']));
				}
