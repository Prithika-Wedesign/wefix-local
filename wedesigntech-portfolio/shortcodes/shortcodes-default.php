	...
	class WDTPortfolioShortcodes {
		...
		function wdt_listings_listing($attrs, $content = null) {
			...
			$attrs = shortcode_atts ( array (
				...
				'carousel_spacebetween_widescreen'=> '',
				'carousel_spacebetween_laptop' => '',
				'carousel_spacebetween_tablet_extra'=>'',
				'carousel_spacebetween_tablet'=>'',
				'carousel_spacebetween_mobile_extra'=>'',
				'carousel_spacebetween_mobile'=>'',
			...
			if($attrs['enable_carousel'] == 'true') {
				...
				$slides_space_between = $attrs['carousel_spacebetween'];
				$slides_to_scroll = 1;
				...
				extract($attrs);
					...
					$carousel_settings = array (
						...
						'carousel_spacebetween' 			=> $slides_space_between,
					);
			
					$active_breakpoints = \Elementor\Plugin::$instance->breakpoints->get_active_breakpoints();
					$breakpoint_keys = array_keys($active_breakpoints);
					$space_between_gaps = array ('desktop' => isset($attrs['carousel_spacebetween']) ? $attrs['carousel_spacebetween'] : 20 );
					...
					foreach($breakpoint_keys as $breakpoint) {
						...
						$breakpoint_gap_str = 'carousel_spacebetween_'.$breakpoint;
						$$breakpoint_gap_str = isset($attrs[$breakpoint_gap_str]) ? $attrs[$breakpoint_gap_str] : $slides_space_between;
						$space_between_gaps[$breakpoint] = $$breakpoint_gap_str;
			...
			if($attrs['enable_carousel'] == 'true') {
				...
				if($attrs['type'] == 'type3' || $attrs['carousel_slidesperview'] > 1) {
			...
			$output = '';
			$output .= '<div class="wdt-listing-output-data-container wdt-direct-list-items wdt-listing-no-map '.esc_attr( $attrs['class'] ).'" '.$listing_carousel_attributes_string.'>';
					...
					if($attrs['carousel_bulletpagination'] == 'true' || $attrs['carousel_arrowpagination'] == 'true') {
						$output .= '<div class="wdt-swiper-pagination-holder">';
							...
							if($attrs['carousel_bulletpagination'] == 'true') {
								$output .= '<div class="wdt-swiper-pagination-fraction-bullets"></div>';
							}
