	...
	class WDTPortfolioListingsPostType {
		...
		function wdt_save_post_meta($post_id) {
			...
			if ( (key_exists('post_type', $_POST)) && ('wdt_listings' == $_POST['post_type']) ) :
				...
				if( isset( $_POST ['wdt_client'] ) && $_POST ['wdt_client'] != '') {
					update_post_meta ( $post_id, 'wdt_client', wdt_sanitize_fields( $_POST ['wdt_client'] ) );
				} else {
					delete_post_meta ( $post_id, 'wdt_client' );
				}

				if( isset( $_POST ['wdt_duration'] ) && $_POST ['wdt_duration'] != '') {
					update_post_meta ( $post_id, 'wdt_duration', wdt_sanitize_fields( $_POST ['wdt_duration'] ) );
				} else {
					delete_post_meta ( $post_id, 'wdt_duration' );
				}

				if( isset( $_POST ['wdt_estimation'] ) && $_POST ['wdt_estimation'] != '') {
					update_post_meta ( $post_id, 'wdt_estimation', wdt_sanitize_fields( $_POST ['wdt_estimation'] ) );
				} else {
					delete_post_meta ( $post_id, 'wdt_estimation' );
				}

				if( isset( $_POST ['wdt_place'] ) && $_POST ['wdt_place'] != '') {
					update_post_meta ( $post_id, 'wdt_place', wdt_sanitize_fields( $_POST ['wdt_place'] ) );
				} else {
					delete_post_meta ( $post_id, 'wdt_place' );
				}
