...
class WDTPortfolioDfListingsListing extends Widget_Base {
	...
	public function get_style_depends() {
		return array ( 'portfolio-swiper', 'wdt-modules-listing', 'wdt-modules-default', 'magnific-popup', 'wdt-listing' );
	}

	public function get_script_depends() {
		return array ( 'portfolio-jquery-swiper', 'wdt-frontend', 'jquery-cookies', 'jquery-magnific-popup' );
	}
	...
	protected function register_controls() {
		...
		$this->start_controls_section( 'listings_listing_carousel_section', array(
			...
			$this->add_control( 'carousel_bulletpagination', array(
				'label'       => esc_html__('Enable Bullet & number Pagination','wdt-portfolio'),
				'type'        => Controls_Manager::SELECT,
				'options'     => array(
					'false' => esc_html__('False','wdt-portfolio'),
					'true'  => esc_html__('True','wdt-portfolio'),
	...
	public function get_listing_carousel_attributes($settings) {
		...
		$slides_space_between = $settings['carousel_spacebetween'];
		$slides_to_scroll = 1;

			// Responsive control carousel
			$carousel_settings = array (
				'carousel_slidesperview' => $slides_to_show,
				'carousel_spacebetween' => (int)$slides_space_between,
			);

			$active_breakpoints = \Elementor\Plugin::$instance->breakpoints->get_active_breakpoints();
			$breakpoint_keys = array_keys($active_breakpoints);

			$space_between_gaps = array ('desktop' => isset($settings['carousel_spacebetween']) ? $settings['carousel_spacebetween'] : 20 );
			...
			foreach($breakpoint_keys as $breakpoint) {
				...
				$$breakpoint_gap_str = isset($settings[$breakpoint_gap_str]) ? $settings[$breakpoint_gap_str] : $slides_space_between;
