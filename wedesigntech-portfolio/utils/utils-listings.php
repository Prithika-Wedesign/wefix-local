    ...
    if(!$dynamic_pagination) {
		...
		if($content != '') {
			...
			if(!$enable_carousel && !$dynamic_pagination) {
                ...
                if($pagination_type != '') {
                    $output .= wdt_listing_ajax_pagination($max_num_pages,$current_page,'wdt-listing-output-data-holder', $output_options['output_div'] ?? 'wdt-listing-output-data-holder',$output_options);
                }
				// Pagination script End
		...
		if($type == 'type1') {
			...
			$output .= '<div class="wdt-listings-item-top-section">';
				...
				$output .= '<div class="wdt-listings-item-top-section-content">';
					...
					$output .= do_shortcode('[wdt_sp_price listing_id="'.esc_attr($listing_id).'" type="type1"/]');
					...
					$output .= '<div class="wdt-listings-item-excerpt-group">';
						...
						$output .= '<div class="wdt-listings-group-button-hover-icon">';
							$output .= '<a class="custom-button-style wdt-button wdt-listing-view-details" href="'.esc_url( get_permalink($listing_id) ).'">'.esc_html__('Discover','wdt-portfolio').''.$view_details_btn.'</a>';
						$output .= '</div>';
					$output .= '</div>';
            ...
            $output .= '<div class="wdt-listings-item-top-section">';
                ...
                $output .= '<div class="wdt-listings-item-top-section-content">';
					...
					$output .= '<div class="wdt-listings-item-title">';
						...
						$output .= '</h4>';
                    $output .= '</div>';
					$output .= '<div class="wdt-listings-item-title-group">';
						$output .= '<div class="wdt-listings-item-title">';
							$output .= '<h4 class="wdt-listings--title">';
								$output .= '<a href="'.esc_url( get_permalink($listing_id) ).'">'.esc_html( get_the_title($listing_id) ).'</a>';
							$output .= '</h4>';
	...
	if($max_num_pages > 1) {
		...
		$output .= '<div class="wdt-pagination wdt-listing-pagination wdt-ajax-pagination '.esc_attr($class).'"  data-functioncall="'.esc_attr( $function_call ).'" data-outputdiv="'.esc_attr( $output_div ).'" data-listing-options="'.esc_js($listing_options).'">';
            ...
            if($item_ids['pagination_type'] == 'loadmore') {
                ...
                if($current_page > 1) {
                    $output .= '<div class="prev-post"><a href="#" data-currentpage="'.esc_attr( $current_page ).'"><span class="fa icon-prev-arrow"></span>&nbsp;'.esc_html__('Prev','wdt-portfolio').'</a></div>';
                }
                ...
                if ($current_page < $max_num_pages) {
                    $output .= '<div class="next-post"><a href="#" data-currentpage="'.esc_attr( $current_page ).'">'.esc_html__('Next','wdt-portfolio').'&nbsp;<span class="fa icon-next-arrow"></span></a></div>';
                }
...
if(!function_exists('breadcrumbs_portfolio_module')) {
    ...
    add_filter( 'wefix_breadcrumbs', 'breadcrumbs_portfolio_module', 10, 1 );
}
