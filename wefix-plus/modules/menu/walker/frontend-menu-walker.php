<?php
if ( ! class_exists( 'WeFixPlusFrontendMenuWalker' ) ) {

	class WeFixPlusFrontendMenuWalker {
		...
		function enqueue_css_assets() {
			wp_enqueue_style( 'dtplugin-nav-menu-animations', WEFIX_PLUS_DIR_URL . 'modules/menu/assets/css/nav-menu-animations.css', false, WEFIX_PLUS_VERSION, 'all');
		}

		function enqueue_js_assets() {
			wp_enqueue_script( 'dtplugin-mega-menu', WEFIX_PLUS_DIR_URL . 'modules/menu/assets/js/mega-menu.js', array(), WEFIX_PLUS_VERSION, true );
		}
	}
}

WeFixPlusFrontendMenuWalker::instance();

if( ! class_exists( 'WeFix_Walker_Nav_Menu' ) ) {

	class WeFix_Walker_Nav_Menu extends Walker_Nav_Menu {
		...
		public function start_el( &$output, $item, $depth = 0, $args = null, $id = 0 ) {
			...
			$item_output .= '<a' . wefix_html_output($attributes) . '>';
				...
				if( !empty( $item->icon ) || !empty( $item->image ) ) {
					...
					if( !empty( $item->image ) ) {
						$item_output .= '<i class="menu-item-image">';
							$item_output .= '<img src="'.esc_url( $item->image ).'" alt="'.esc_attr( 'Image', 'wefix-plus' ).'" title="'.esc_attr( 'Image', 'wefix-plus' ).'"/>';
						$item_output .= '</i>';
					}
				}
