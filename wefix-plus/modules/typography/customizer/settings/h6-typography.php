...
if( !class_exists( 'WeFixPlusH6Settings' ) ) {
    class WeFixPlusH6Settings {
        ...
        function __construct() {
            $this->selector = apply_filters( 'wefix_h6_selector', array( 'h6' ) );
            $this->settings = wefix_customizer_settings('h6_typo');

            add_filter( 'wefix_plus_customizer_default', array( $this, 'default' ) );
            add_action( 'customize_register', array( $this, 'register' ), 20);

            add_filter( 'wefix_h6_typo_customizer_update', array( $this, 'h6_typo_customizer_update' ) );
        ...
        function register( $wp_customize ) {
            ...
            $wp_customize->add_section(
                new WeFix_Customize_Section(
                    ...
                    'site-h6-section',
                    array(
                        'title'    => esc_html__('H6 Typography', 'wefix-plus'),
             ...
             */
                $wp_customize->add_setting(
                    WEFIX_CUSTOMISER_VAL . '[h6_typo]', array(
                        'type'    => 'option',
                    )
                );
                ...
                $wp_customize->add_control(
                    new WeFix_Customize_Control_Typography(
                        $wp_customize, WEFIX_CUSTOMISER_VAL . '[h6_typo]', array(
                            'type'    => 'wdt-typography',
                            'section' => 'site-h6-section',
                            'label'   => esc_html__( 'H6 Tag', 'wefix-plus'),
                        )
                    )
                );
             ...
             */
                $wp_customize->add_setting(
                    WEFIX_CUSTOMISER_VAL . '[h6_color]', array(
                        'default' => '',
                        'type'    => 'option',
                    )
                );
                ...
                $wp_customize->add_control(
                    new WP_Customize_Color_Control(
                        $wp_customize, WEFIX_CUSTOMISER_VAL . '[h6_color]', array(
        ...
        function h6_typo_customizer_update( $defaults ) {
            $h6_typo = wefix_customizer_settings( 'h6_typo' );
            if( !empty( $h6_typo ) ) {
                return  $h6_typo;
            }
            return $defaults;
        ...
        function base_style( $style ) {
            $css   = '';
            $color = wefix_customizer_settings('h6_color');
