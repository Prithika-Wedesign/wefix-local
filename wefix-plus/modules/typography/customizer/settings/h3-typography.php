...
if( !class_exists( 'WeFixPlusH3Settings' ) ) {
    class WeFixPlusH3Settings {
        ...
        function __construct() {
            $this->selector = apply_filters( 'wefix_h3_selector', array( 'h3' ) );
            $this->settings = wefix_customizer_settings('h3_typo');

            add_filter( 'wefix_plus_customizer_default', array( $this, 'default' ) );
            add_action( 'customize_register', array( $this, 'register' ), 20);

            add_filter( 'wefix_h3_typo_customizer_update', array( $this, 'h3_typo_customizer_update' ) );
        ...
        function register( $wp_customize ) {
            ...
            $wp_customize->add_section(
                new WeFix_Customize_Section(
                    ...
                    'site-h3-section',
                    array(
                        'title'    => esc_html__('H3 Typography', 'wefix-plus'),
             ...
             */
                $wp_customize->add_setting(
                    WEFIX_CUSTOMISER_VAL . '[h3_typo]', array(
                        'type'    => 'option',
                    )
                );
                ...
                $wp_customize->add_control(
                    new WeFix_Customize_Control_Typography(
                        $wp_customize, WEFIX_CUSTOMISER_VAL . '[h3_typo]', array(
                            'type'    => 'wdt-typography',
                            'section' => 'site-h3-section',
                            'label'   => esc_html__( 'H3 Tag', 'wefix-plus'),
                        )
                    )
                );
             ...
             */
                $wp_customize->add_setting(
                    WEFIX_CUSTOMISER_VAL . '[h3_color]', array(
                        'default' => '',
                        'type'    => 'option',
                    )
                );
                ...
                $wp_customize->add_control(
                    new WP_Customize_Color_Control(
                        $wp_customize, WEFIX_CUSTOMISER_VAL . '[h3_color]', array(
        ...
        function h3_typo_customizer_update( $defaults ) {
            $h3_typo = wefix_customizer_settings( 'h3_typo' );
            if( !empty( $h3_typo ) ) {
                return  $h3_typo;
            }
            return $defaults;
        ...
        function base_style( $style ) {
            $css   = '';
            $color = wefix_customizer_settings('h3_color');
